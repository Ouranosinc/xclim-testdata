...
# Base Path for the two version of ERA5. This is the structure on Ouranos' servers.
NAMpath = base_path + '/era5_rechunked/NAM/*_era5_reanalysis_hourly_199[0123]_NAM.nc'
GLBpath = base_path + '/era5_rechunked/*_era5_reanalysis_hourly_199[0123].nc'
...
if __name__ == '__main__':
    ...
    raw_glb = xr.open_mfdataset(GLBpath, chunks={'time': 2928, 'latitude': 25, 'longitude': 50})
    raw_glb['longitude'] = raw_glb.longitude - 360
    ...
    lon = xr.DataArray(
        ...
        attrs={'standard_name': 'longitude', 'units': 'degree_east', 'long_name': 'longitude'}
    )
    lat = xr.DataArray(
        [44.5, 45.5, 63.75, 52.0, 48.5],
        dims=('location',),
        coords={'location': location},
        attrs={'standard_name': 'latitude', 'units': 'degree_north', 'long_name': 'latitude'}
    )
    ...
    rh = convert_units_to(
        xc.atmos.relative_humidity_from_dewpoint(tas=hrly.tas, dtas=hrly.dtas),
        '1'
    ).resample(time='D').mean(keep_attrs=True)
    rh.attrs.update(
    ...
    sund = convert_units_to(
        xc.core.units.to_agg_units(
            (hrly.rsds > 120).resample(time='D').sum(),
            hrly.rsds,
            'count'
        ),
        's'
